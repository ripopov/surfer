//! Native Surfer chrome, adapted from the egui-table demo.
use eframe::egui::{
...
pub fn header(
    ...
    title: &str,
    msgs: &mut Vec<crate::message::Message>,
) {
    let radius = corner_radius(root.ctx());
    egui::Panel::top("window-header")
        ...
        .show(root, |ui| {
            ...
            if controls_left > rect.left() + 260.0 {
                p.with_clip_rect(Rect::from_min_max(
                    rect.min,
                    pos2(controls_left - 12.0, rect.bottom()),
                ))
                .text(
                    pos2(rect.left() + 110.0, rect.center().y + 1.0),
                    Align2::LEFT_CENTER,
                    title,
...
fn selector(
    ...
    let mut selected = theme.theme_name.clone();
    egui::ComboBox::from_id_salt("chrome-theme")
        .width(190.0)
        .height(360.0)
    ...
    if selected != theme.theme_name {
        msgs.push(crate::message::Message::SelectTheme(
...
mod tests {
    ...
    fn frame(
        ...
        let mut output = ctx.run_ui(input, |ui| {
            header(
                ui,
                &crate::config::SurferTheme::builtin(None).unwrap(),
                "Waveform viewer",
                &mut messages,
            );
        ...
        assert!(
            messages.is_empty(),
            "window gestures must not select a theme"
        );
