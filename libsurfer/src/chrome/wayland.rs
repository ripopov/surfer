...
impl Decorations {
    ...
    pub fn update(&mut self, geometry: Geometry, changed: bool) -> Result<()> {
        ...
        if geometry.radius == 0 {
            ...
            let fd =
                rustix::fs::memfd_create(c"surfer-window-shadow", rustix::fs::MemfdFlags::CLOEXEC)?;
...
fn shadow_pixels(g: Geometry) -> Result<(i32, i32, Vec<u8>)> {
    let width = g
        ...
        .ok_or_else(|| eyre::eyre!("shadow width overflow"))?;
    let height = g
        .height
        .checked_add(SHADOW * 2)
        .ok_or_else(|| eyre::eyre!("shadow height overflow"))?;
    let len = i64::from(width)
        .checked_mul(i64::from(height))
        .and_then(|pixels| pixels.checked_mul(4))
        .ok_or_else(|| eyre::eyre!("shadow allocation overflow"))?;
    eyre::ensure!(
