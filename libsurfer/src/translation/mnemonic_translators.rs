...
lazy_static! {
    static ref KIND_COLOR_KEYWORDS: HashMap<&'static str, ValueKind> = {
...
impl fmt::Display for MnemonicParseError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            MnemonicParseError::LineError { line, content, message } => {
...
pub fn parse_content_with_default_name(
    ...
    for (line_num, line_str) in lines_iter {
        ...
        if name.is_none() {
            if starts_with_ignore_ascii_case(trimmed, "name:") {
        ...
        if bits.is_none() && starts_with_ignore_ascii_case(trimmed, "bits:") {
            bits = Some(parse_bits_line(trimmed)?);
            continue;
        }

        // Otherwise, it's a data entry line
...
fn parse_color_kind(token: &str) -> Result<ValueKind, MnemonicParseError> {
    ...
    KIND_COLOR_KEYWORDS
        .get(lower.as_str())
        .copied()
        .ok_or_else(|| MnemonicParseError::UnknownColor(token.to_string()))
}
...
mod tests {
    ...
    fn error_variants_coverage() {
        ...
        match parse_color_kind("not_a_color") {
            Err(MnemonicParseError::UnknownColor(s)) => assert_eq!(s, "not_a_color"),
