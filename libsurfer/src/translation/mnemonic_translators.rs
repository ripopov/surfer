...
impl fmt::Display for MnemonicParseError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            MnemonicParseError::Io(e) => write!(f, "IO error: {e}"),
            MnemonicParseError::InvalidBitsLineFormat => write!(f, "Invalid Bits: line format"),
            MnemonicParseError::InvalidBitsValue(v) => write!(f, "Invalid bits value: {v}"),
            MnemonicParseError::InvalidHex(tok) => write!(f, "Invalid hex number: {tok}"),
            MnemonicParseError::UnknownColor(c) => write!(f, "Unknown color: {c}"),
            MnemonicParseError::BinaryTooWide { value, required, specified } => write!(
                f,
                "Binary string '{value}' requires {required} bits, but only {specified} bits specified"
            ),
            MnemonicParseError::StringLengthMismatch { value, value_len, expected } => write!(
                f,
                "String '{value}' has {value_len} characters, expected {expected} characters to match bit width"
            ),
            MnemonicParseError::MissingSecondColumn => write!(f, "Missing second column"),
            MnemonicParseError::EmptyLine => write!(f, "Empty line"),
            MnemonicParseError::LineError { line, message } => {
...
impl std::error::Error for MnemonicParseError {}

impl From<std::io::Error> for MnemonicParseError {
    fn from(e: std::io::Error) -> Self {
        MnemonicParseError::Io(e.to_string())
    }
}
...
pub fn parse_content_with_default_name(
    ...
    // Skip leading blank/comment lines (comments start with //)
    ...
    if line_idx < lines.len() {
        ...
        if !first_line.starts_with("//") {
            if starts_with_ignore_ascii_case(first_line, "name:") {
    ...
    if line_idx < lines.len() {
        ...
        if starts_with_ignore_ascii_case(current_line, "bits:") {
    ...
    for (idx, line) in lines.iter().enumerate().skip(line_idx) {
        ...
        // Strip inline comments starting with // outside quotes
        line = strip_inline_comment(line).trim();
        if line.is_empty() {
            continue;
        }
...
fn looks_like_name(line: &str) -> bool {
    ...
    // If it starts with "bits:", it's not a string
    if starts_with_ignore_ascii_case(trimmed, "bits:") {
        return false;
    }

    // If it starts with 0x or a digit, probably not a string
    ...
    // If it has very few tokens (not quoted), might be a string
...
fn parse_color_kind(token: &str) -> Result<ValueKind, MnemonicParseError> {
    ...
    // Try value kinds and ecolor::Color32 named colors
    match token.to_lowercase().as_str() {
        "normal" => Ok(ValueKind::Normal),
        "default" => Ok(ValueKind::Normal),
        "undef" => Ok(ValueKind::Undef),
        "highimp" => Ok(ValueKind::HighImp),
        "warn" => Ok(ValueKind::Warn),
        "dontcare" => Ok(ValueKind::DontCare),
        "weak" => Ok(ValueKind::Weak),
        "error" => Ok(ValueKind::Error),
        "black" => Ok(ValueKind::Custom(Color32::BLACK)),
        "white" => Ok(ValueKind::Custom(Color32::WHITE)),
        "red" => Ok(ValueKind::Custom(Color32::RED)),
        "green" => Ok(ValueKind::Custom(Color32::GREEN)),
        "blue" => Ok(ValueKind::Custom(Color32::BLUE)),
        "yellow" => Ok(ValueKind::Custom(Color32::YELLOW)),
        "cyan" => Ok(ValueKind::Custom(Color32::CYAN)),
        "magenta" => Ok(ValueKind::Custom(Color32::MAGENTA)),
        "gray" | "grey" => Ok(ValueKind::Custom(Color32::GRAY)),
        "light_gray" | "light_grey" => Ok(ValueKind::Custom(Color32::LIGHT_GRAY)),
        "dark_gray" | "dark_grey" => Ok(ValueKind::Custom(Color32::DARK_GRAY)),
        "brown" => Ok(ValueKind::Custom(Color32::BROWN)),
        "dark_red" => Ok(ValueKind::Custom(Color32::DARK_RED)),
        "light_red" => Ok(ValueKind::Custom(Color32::LIGHT_RED)),
        "orange" => Ok(ValueKind::Custom(Color32::ORANGE)),
        "light_yellow" => Ok(ValueKind::Custom(Color32::LIGHT_YELLOW)),
        "khaki" => Ok(ValueKind::Custom(Color32::KHAKI)),
        "dark_green" => Ok(ValueKind::Custom(Color32::DARK_GREEN)),
        "light_green" => Ok(ValueKind::Custom(Color32::LIGHT_GREEN)),
        "dark_blue" => Ok(ValueKind::Custom(Color32::DARK_BLUE)),
        "light_blue" => Ok(ValueKind::Custom(Color32::LIGHT_BLUE)),
        "purple" => Ok(ValueKind::Custom(Color32::PURPLE)),
        "gold" => Ok(ValueKind::Custom(Color32::GOLD)),
        _ => Err(MnemonicParseError::UnknownColor(token.to_string())),
    }
}
