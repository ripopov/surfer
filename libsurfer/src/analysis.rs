use egui::{Button, Context, Pos2, RichText, Ui, Window};
use egui_graphs::{
...
#[derive(Debug, Deserialize, Hash, Eq, PartialEq, Sequence)]
...
impl AnalysisWindow {
    ...
    pub fn default_visibility_map() -> AnalysisWindowVisibilityMap {
        ...
        for analysis_type in enum_iterator::all::<AnalysisWindow>() {
            map.insert(analysis_type, false);
        }
        map
    }
}
...
pub fn compute_signal_transition_stats(
    ...
    let last = data.last().cloned();

    for (time, value) in data {
        if let Some((prev_time, prev_value)) = &prev {
            let dt = time.clone() - prev_time;
            *state_times
    ...
    if let Some((last_time, last_value)) = last {
        let dt = total_time_bigint.to_biguint().unwrap() - last_time;
    ...
    for (idx, (node, _)) in state_times.iter().enumerate() {
        ...
        nodemapping.insert(
            ...
            graph.add_node_with_label_and_location(
                (),
                label,
                // Probably better to use a random layout initially, but requires additional dependencies
...
impl SystemState {
    pub(crate) fn handle_open_analysis_window(
        ...
        match item {
            DisplayedItem::Variable(v) => {
                ...
                let total_time_bigint = waves.num_timestamps().unwrap_or_default();

                let (transitions, state_times) =
                    compute_signal_transition_stats(data, &total_time_bigint);
                ...
                let total_time = total_time_bigint.to_f64().unwrap_or_default().max(1.0);
                let meta = waves
                    .inner
                    .as_waves()
                    .unwrap()
    ...
    fn draw_histogram_window(&mut self, ctx: &Context, msgs: &mut Vec<Message>) {
        let mut open = true;
    ...
    fn draw_state_window(&mut self, ctx: &Context, msgs: &mut Vec<Message>) {
            ...
            Window::new(format!("State transitions - {var_name}"))
                ...
                .show(ctx, |ui| {
                    if let Some(stats) = self.analysis_statistics.as_mut() {
                        ...
                        for ((edge_value_1, edge_value_2), eidx) in stats.edgemapping.iter() {
                            if let Some(edge) = stats.graph.edge_mut(*eidx) {
                                if let Some(_color) = get_edge_color(
                                    &(edge_value_1, edge_value_2),
                                    &current_value,
                                    &previous_value,
                                ) {
    ...
    pub fn draw_analysis_windows(&mut self, ctx: &Context, msgs: &mut Vec<Message>) {
        if let Some(visible) = self.show_analysis_window.get(&AnalysisWindow::Histogram) {
            if *visible {
                self.draw_histogram_window(ctx, msgs);
            }
        }
        if let Some(visible) = self.show_analysis_window.get(&AnalysisWindow::State) {
            if *visible {
                self.draw_state_window(ctx, msgs);
            }
        }
    }
}
...
pub fn analysis_menu(msgs: &mut Vec<Message>, ui: &mut Ui, vidx: VisibleItemIndex) {
    ui.menu_button("Analysis", |ui| {
        for analysis_type in enum_iterator::all::<AnalysisWindow>() {
            if ui.button(format!("{:?}", analysis_type)).clicked() {
                msgs.push(Message::OpenAnalysisWindow(Some(vidx), analysis_type));
            }
        }
    });
}
