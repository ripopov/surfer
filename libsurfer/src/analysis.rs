...
impl SystemState {
    pub(crate) fn handle_open_analysis(
        &mut self,
        vidx: Option<VisibleItemIndex>,
        mode: AnalysisWindows,
    ) -> Option<()> {
        ...
        match item {
            DisplayedItem::Variable(v) => {
                ...
                self.statistics = Some(VariableStatistics {
                ...
                match mode {
                    AnalysisWindows::Histogram => {
                        self.show_histogram_window = true;
                    }
                    AnalysisWindows::State => {
                        self.show_state_window = true;
                    }
                }
            }
    ...
    pub fn draw_histogram_window(&mut self, ctx: &Context, msgs: &mut Vec<Message>) {
        ...
        if !open {
            msgs.push(Message::SetHistogramWindowVisible(false));
        }
    }
    ...
    pub fn draw_state_window(&mut self, ctx: &Context, msgs: &mut Vec<Message>) {
        if let Some(waves) = self.user.waves.as_ref() {
            let mut open = true;
            let var_name = self.statistics.as_ref().map_or(String::new(), |stats| {
            ...
            if !open {
                msgs.push(Message::SetStateWindowVisible(false));
            }
        }
    }
}
