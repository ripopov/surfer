...
impl WaveData {
    ...
    fn load_streams_for_displayed_items(
        ...
        let stream_ids: Vec<ftr_parser::types::StreamId> = displayed_items
            .values()
    ...
    pub fn update_with_items(
        ...
        self.displayed_items = if let Some(waves) = self.inner.as_waves() {
            Self::update_displayed_items(waves, &mut self.items_tree, new_items, true, translators)
        } else {
    ...
    pub fn get_top_item(&self) -> usize {
        ...
        self.drawing_infos
            .iter()
            .enumerate()
            .find(|(_, di)| di.top() >= visible_top - 1.)
