...
// Waveform + Konata time-axis synchronization across two fused sources:
// examples/kanata-wave.vcd (generated by examples/kanata_wave_gen.py with the
// same 1 ns timescale and 0..4542 ns span as the trace) is the primary
// waveform source, and examples/kanata-sample-2.ftr is added as a second
// source. Both panes show the same time window via "Synchronize scroll".
snapshot_ui! {konata_vcd_ftr_sync_view, || {
    let mut state = SystemState::new_default_config()
        .unwrap()
        .with_params(StartupParams {
            waves: Some(WaveSource::File(
                get_project_root()
                    .unwrap()
                    .join("examples/kanata-wave.vcd")
                    .try_into()
                    .unwrap(),
            )),
            additional_waves: vec![WaveSource::File(
                get_project_root()
                    .unwrap()
                    .join("examples/kanata-sample-2.ftr")
                    .try_into()
    ...
    wait_for_waves_fully_loaded(&mut state, 10);
    assert_eq!(
        state.user.waves.as_ref().unwrap().source_count(),
        2,
        "both the VCD and the FTR source should be loaded"
    );
    ...
    for message in [
        ...
        // An explicit ruler (the snapshot harness forces the default timeline
        // off) plus a representative slice of the generated periodic signals.
        Message::AddTimeLine(None),
        Message::AddVariablesFromSource(
            SourceId::default(),
            vec![
                VariableRef::from_hierarchy_string("tb.clk_fast"),
                VariableRef::from_hierarchy_string("tb.clk_slow"),
                VariableRef::from_hierarchy_string("tb.sine_a"),
                VariableRef::from_hierarchy_string("tb.sine_b"),
                VariableRef::from_hierarchy_string("tb.counter"),
                VariableRef::from_hierarchy_string("tb.pwm"),
                VariableRef::from_hierarchy_string("tb.onehot"),
            ],
        ),
        // Render the sine waves and the sawtooth counter as analog curves.
        Message::SetAnalogSettings(
            MessageTarget::Explicit(VisibleItemIndex(3)),
            Some(crate::displayed_item::AnalogSettings {
                render_style: crate::displayed_item::AnalogRenderStyle::Interpolated,
                y_axis_scale: crate::displayed_item::AnalogYAxisScale::Viewport,
            }),
        ),
        Message::ItemHeightScalingFactorChange(MessageTarget::Explicit(VisibleItemIndex(3)), 4.0),
        Message::SetAnalogSettings(
        ...
        Message::ItemHeightScalingFactorChange(MessageTarget::Explicit(VisibleItemIndex(4)), 4.0),
        Message::SetAnalogSettings(
            MessageTarget::Explicit(VisibleItemIndex(5)),
            Some(crate::displayed_item::AnalogSettings {
                render_style: crate::displayed_item::AnalogRenderStyle::Step,
                y_axis_scale: crate::displayed_item::AnalogYAxisScale::Viewport,
            }),
        ),
        Message::ItemHeightScalingFactorChange(MessageTarget::Explicit(VisibleItemIndex(5)), 2.0),
        // Instruction transactions from the FTR source in the waveform pane,
        // and the Konata pipeline view of the same generator.
        Message::AddStreamOrGeneratorFromSource(
            SourceId(1),
    ...
    let tile_id = *state.user.konata_tiles.keys().next().unwrap();
    {
        ...
        tile.config.sync_group = Some(0);
        tile.config.splitter_px = 140.0;
        // A window wide enough (~123 ticks) that the pipeline diagonal and
        // several periods of the generated signals are visible at once.
        tile.viewport.top_visible_row = 96.0;
        tile.viewport.set_left_tick(640);
        tile.viewport.px_per_tick = 4.0;
        tile.viewport.row_height_px = 6.0;
    }
    // Approximate the Konata canvas width in the horizontal split and run the
    // per-frame sync driver directly (the snapshot harness only calls `draw`),
    // coupling the waveform viewport onto the Konata tile's time window.
    state
        .konata_runtime
        .entry(tile_id)
        .or_default()
...
snapshot_ui!(table_transaction_tables_restored_from_state_file, || {
    ...
    state.update(Message::AddStreamOrGenerator(
        TransactionStreamRef::new_stream(StreamId(1), "pipelined_stream".to_string()),
    ));
