...
/// Builds persisted column config entries from a runtime schema.
#[must_use]
pub fn column_configs_from_schema(schema: &TableSchema) -> Vec<TableColumnConfig> {
    schema
        .columns
        .iter()
        .map(|column| TableColumnConfig {
            key: column.key.clone(),
            width: column.default_width,
            visible: column.default_visible,
            resizable: column.default_resizable,
        })
        .collect()
}
...
/// Returns true when the search spec is the default "no filter" value.
#[must_use]
pub fn is_default_search_spec(spec: &TableSearchSpec) -> bool {
    spec.text.is_empty()
        && !spec.case_sensitive
        && spec.mode == TableSearchMode::Contains
        && spec.column.is_none()
}

/// Normalizes a filter list by dropping empty clauses and removing duplicates
/// while preserving first-seen order.
#[must_use]
pub fn normalize_search_specs(filters: Vec<TableSearchSpec>) -> Vec<TableSearchSpec> {
    filters
        .into_iter()
        .filter(|filter| !filter.text.is_empty())
        .fold(Vec::new(), |mut deduped, filter| {
...
#[must_use]
fn navigation_no_target_result() -> NavigationResult {
    NavigationResult {
        target_row: None,
...
#[must_use]
fn navigation_single_row_result(target_row: TableRowId) -> NavigationResult {
    let mut new_selection = TableSelection::new();
...
pub fn navigate_up(
    ...
    if visible_rows.is_empty() {
        return navigation_no_target_result();
    }
    ...
    navigation_single_row_result(target_row)
}
...
pub fn navigate_page_up(
    ...
    navigation_single_row_result(visible_rows[target_idx])
}
...
pub fn navigate_home(visible_rows: &[TableRowId]) -> NavigationResult {
    ...
    navigation_single_row_result(visible_rows[0])
}
...
pub fn navigate_end(visible_rows: &[TableRowId]) -> NavigationResult {
    ...
    navigation_single_row_result(visible_rows[visible_rows.len() - 1])
}
