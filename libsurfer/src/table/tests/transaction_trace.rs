...
fn load_ftr_state() -> SystemState {
    ...
    state.update(Message::AddStreamOrGenerator(
        TransactionStreamRef::new_stream(StreamId(2), "addr_stream".to_string()),
    ));
    state.update(Message::AddStreamOrGenerator(
        TransactionStreamRef::new_stream(StreamId(3), "data_stream".to_string()),
    ));
    wait_for_waves_fully_loaded(&mut state, 10);
...
fn test_generator_ref() -> crate::transaction_container::TransactionStreamRef {
    crate::transaction_container::TransactionStreamRef::new_gen(
        StreamId(1),
        GeneratorId(4),
        "read".to_string(),
    )
...
fn open_transaction_table_for_stream_creates_multiple_tiles() {
    ...
    let stream_ref = TransactionStreamRef::new_stream(StreamId(1), "pipelined_stream".to_string());
