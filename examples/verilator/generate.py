...
def main():
    ...
    parser.add_argument('--verilator', type=Path, default=VTR / 'bench/build/verilator-submodule/install/bin/verilator')
    parser.add_argument('--build-dir', type=Path, default=VTR / 'bench/build/surfer-examples')
    args = parser.parse_args()
    env = dict(os.environ, VTR_INCLUDE=str(VTR / 'crates/vtr-capi/include'),
               VTR_LIBDIR=str(VTR / 'target/release'))
